...
public class MainD14_2 {
	...
	void run() throws IOException {
		List<List<Pair<Integer, Integer>>> pairs = Files.readAllLines(Paths.get("andre/d14/input.txt")).stream().map(MainD14_2::split).toList();

		int xNr = maxX + 400;
		int yNr = maxY + 3;

		grid = new ArrayList<>();
		IntStream.range(0, yNr).forEach(i -> {
			String b = i == (maxY + 2) ? "#" : ".";
			grid.add(new ArrayList<>(IntStream.range(0, xNr).mapToObj(k -> b).toList()));
		});
		...
		for (List<Pair<Integer, Integer>> lp : pairs) {
			...
			for (Pair<Integer, Integer> p : lp) {
				...
				if (lastX == currX) {
					...
					System.out.println("failed for :" + Pair.with(lastX, lastY) + " " + p);
				}
