...
//		printGraph(undirectedGraph, name + "-simple.dot");
		...
		Set<Valve> alreadyOpened = Set.copyOf(bestPath);

		simplifyGraph2(graph, start, alreadyOpened);
		printGraph(graph, name + "-simple-part2.dot");
		...
		Set<List<Valve>> paths2 = dfs2(start, new ArrayList<>(), 1, 0);
		Pair<Integer, List<Valve>> best2 = findBest(start, paths2);
		int max2 = best2.getValue0();
		List<Valve> bestPath2 = best2.getValue1();

		// 2859 too low
		// 2899 too low
		System.out.println("found best with " + max + ": " + bestPath);
		System.out.println("found best2 with " + max2 + ": " + bestPath2);
		System.out.println("found total " + (max2 + max));
	...
	int pathMax(Valve start, List<Valve> path, Map<String, Integer> distanceMap) {
		...
		for (var v : path) {
			...
			if (minutes >= 26) break;
			sum += ((26 - minutes + 1) * v.flowRate);
			last = v;
		}
		return sum;
	}

	Set<List<Valve>> dfs3(final Valve node, final Valve nodeElefant, final List<Valve> path, final List<Valve> elefant, int minutes, int flowRate) {
	...
	Set<List<Valve>> dfs2(final Valve node, final List<Valve> path, int minutes, int flowRate) {
		...
		if (minutes >= 26 || flowRate == maxFlowRate || path.size() >= maxPathSize) {
			allPaths.add(path);
			return allPaths;
		}
	...
	void simplifyGraph2(DefaultUndirectedWeightedGraph<Valve, ExtendedWeightedEdge> graph, Valve start, Set<Valve> openedValves) {
		for (var v : new ArrayList<>(graph.vertexSet())) {
			if (!openedValves.contains(v) || v.equals(start)) continue;
			Set<ExtendedWeightedEdge> edges = graph.edgesOf(v);
			for (var e : edges) {
				var source = e.getSource().equals(v) ? e.getTarget() : e.getSource();
				for (var e2 : edges) {
